# syncode-persistence
> SQLite event store + read-model projections + snapshots (concrete CQRS write/read side). **L1** · 1816 LOC · 17 tests
...
- `event_store.rs` (438 LOC) — append/replay with optimistic concurrency.
- `adapters.rs` (487 LOC) — `SqliteEventRepository` + `SqliteReadModelRepository` (impl the core ports).
- `projections.rs` (623 LOC) — `ProjectionManager` (materialized views).
- `snapshot.rs` (163 LOC) — snapshot CRUD.
- `migrations/mod.rs` (4 LOC) — **stub** (Phase 0.3 TODO).
...
- **Read (5 projection tables):** `view_projects`, `view_threads`, `view_turns`, `view_messages`, `view_activities`.
