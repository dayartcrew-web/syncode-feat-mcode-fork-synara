...
>
> ✅ **DONE (2026-07-03): clone + re-wire COMPLETE.** The clone-and-rewire strategy below was executed end-to-end (PR #6–#32): MCode `apps/web` (753 files) vendored + flattened; transport re-wired Effect-RPC → Syncode JSON-RPC (Effect-free, `MCODE_TO_SERVED` 88 mappings); contracts shim complete (139 Tier-3 symbols + 44-event union + branded IDs); **frontend tsc 0, suite 2128/0**. A standalone WS backend (`cargo run -p syncode-ws --bin server`) serves **97 RPCs** across all panels (shell/git/server/terminal/automation/provider/stats) — read-side REAL where Syncode has the subsystem, STUB-defaults elsewhere. Terminal streams live output; automations execute (ProcessRunExecutor); LLM ops (compactThread/summarizeDiff/recap) work via provider CLI (**no API key**). For the authoritative REAL-vs-STUB matrix see [`STATUS.md`](./STATUS.md). Body below is the original analysis, retained.
