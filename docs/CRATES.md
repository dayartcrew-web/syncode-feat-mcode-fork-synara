...
| [`syncode-orchestration`](../.masday/intel/crates/syncode-orchestration.md) | L2 | CQRS engine — 48 Commands, Decider, Projector, Reactors, ApplicationService (56 use cases) + **agentic subsystems** (Critic + DAG runtime + Workflow executor) | ~12500 | 215+ | ✅ |
| [`syncode-memory`](../crates/syncode-memory/MEMORY.md) | L1 | Persistent interaction context — `MemoryProvider` trait + SQLite-backed default + `HybridMemoryProvider` for multi-store (vector/graph/episodic) composition | ~1100 | 16 | ✅ (PR #207) |
...
| [`syncode-ws`](../.masday/intel/crates/syncode-ws.md) | L3 | WebSocket JSON-RPC server + push bus + channels + authz + **standalone bin** (`src/bin/server.rs`) + **97 served RPCs** (all MCode domains) + terminal live-push + LLM-via-provider-CLI (`src/llm.rs`) + skills_catalog (10 origins) | ~5500 | 132 | ✅ backend (clone+rewire) |
