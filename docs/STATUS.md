...
- **P2-2 AI-completion production harness** — the `evaluate_completion_policy` evaluator (`automation/completion_eval.rs`) is complete and tested, but the MCode-style always-on host (bounded queue cap 100 + 2 worker fibers + 30s timeout) that drives it is absent. Evaluator needs no changes; only the harness is missing.
- **P2-8 worktree working-dir wiring** — `WorktreeManager` creates+cleans real git worktrees, but the worktree path is not yet threaded into `DispatchRequest.working_dir`; standalone runs are isolated on create/cleanup but the command itself isn't executed inside the worktree.
