 ...
 */
export const WORKTREE_BRANCH_PREFIX = "mcode";
...
const MCODE_BRANCH_FALLBACK = "update";
...
export function buildMCodeBranchName(preferredBranch?: string | null): string {
  const normalizedExisting =
    preferredBranch?.trim().replace(/^(codex|t3code|dpcode|mcode)\//i, "") ?? "";
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || MCODE_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueMCodeBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildMCodeBranchName(preferredBranch);
  const existingNames = new Set(existingBranchNames.map((branch) => branch.toLowerCase()));
