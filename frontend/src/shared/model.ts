...
const MODEL_SLUG_SET_BY_PROVIDER: Record<ProviderKind, ReadonlySet<ModelSlug>> & Record<string, ReadonlySet<ModelSlug>> = {
  claudeAgent: new Set(MODEL_OPTIONS_BY_PROVIDER.claudeAgent.map((option) => option.slug)),
  claude: new Set(MODEL_OPTIONS_BY_PROVIDER.claudeAgent.map((option) => option.slug)),
...
export function getGeminiThinkingModelAlias(
  ...
  if (kind === "level" && nextOptions.thinkingLevel) {
    return `mcode-gemini-${base}-thinking-level-${nextOptions.thinkingLevel.toLowerCase()}`;
  }
  if (kind === "budget" && nextOptions.thinkingBudget !== undefined) {
    ...
    return `mcode-gemini-${base}-thinking-budget-${budget}`;
  }
  return null;
}
...
export function normalizeModelSlug(
  ...
  const aliases = MODEL_SLUG_ALIASES_BY_PROVIDER[provider] as
    | Record<string, ModelSlug>
    | undefined;
  const aliased =
    aliases && Object.prototype.hasOwnProperty.call(aliases, providerScopedModel)
