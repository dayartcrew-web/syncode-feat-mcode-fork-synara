...
export const MCODE_WS_TRANSPORT_STATE_EVENT = "mcode:ws-transport-state";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(MCODE_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(MCODE_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(MCODE_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
