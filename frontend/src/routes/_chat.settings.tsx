...
function SettingsRouteView() {
  ...
  // Refresh provider statuses when the settings page mounts, so the
  // availability pills reflect the current CLI state instead of a stale boot snapshot.
  useEffect(() => {
    refreshProviderStatusesNow();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleRefreshProviders = useCallback(() => {
    setIsRefreshingProviders(true);
    // The refresh hook updates the react-query cache; flip the spinner off on the next tick.
    refreshProviderStatusesNow();
    setTimeout(() => setIsRefreshingProviders(false), 800);
  }, [refreshProviderStatusesNow]);
  ...
  async function sendTestNotification() {
    ...
    const notification = new Notification(title, { body, tag: "mcode:test-notification" });
    ...
    <div className="space-y-6">
      <section className={SETTINGS_PANEL_SECTION_CLASS_NAME}>
        ...
        <SettingsCard>
          <SettingsRow
            title="Theme"
            description="Choose how MCode looks across the app."
            resetAction={
              theme !== "system" ? (
    ...
    <div ref={providerUpdatesRef} id={SETTINGS_TARGETS.providerUpdates}>
      <SettingsSection title="Updates">
        <SettingsRow
          ...
          description="Update installed provider tools that MCode can safely update."
          status={
        ...
        <SettingsRow
          ...
          control={
            <Button
              ...
              title="Re-check installed providers"
            >
              <RefreshCwIcon
                className={cn("size-3.5", isRefreshingProviders && "animate-spin")}
              />
              {isRefreshingProviders ? "Checking" : "Refresh"}
            </Button>
          }
          resetAction={
                      ...
                      <div className="flex min-h-11 items-center gap-2 px-3 py-2">
                        ...
                        >
                          ...
                          {providerStatus ? (
                            ...
                            >
                              {providerStatus.available ? "Installed" : "Not available"}
                            </span>
                          ) : null}
                          {isDirty ? (
                            <span className="shrink-0 text-[11px] text-muted-foreground">
                            ...
                            updateAdvisory?.status === "behind_latest" ? (
                              <div className="text-xs text-muted-foreground">
                                {updateAdvisory.canUpdate && updateAdvisory.updateCommand ? (
                                  ...
                                  "A newer version is available, but MCode could not identify a safe one-click update command for this installation."
                                )}
                              </div>
                            ) : null}
