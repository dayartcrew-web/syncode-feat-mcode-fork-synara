...
describe("resolveDefaultCreateBranchName", () => {
  it("uses MCode as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "mcode/fix-toast-copy");
  });
  ...
  it("normalizes an existing legacy dpcode namespace", () => {
    ...
    assert.equal(branch, "mcode/refine-toolbar-actions");
  });

  it("preserves nested namespaces under MCode", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "mcode/feature/refine-toolbar-actions");
  });

  it("increments suffix when the MCode branch already exists", () => {
    const branch = resolveDefaultCreateBranchName(
      ["main", "mcode/fix-toast-copy", "mcode/fix-toast-copy-2"],
      "fix toast copy",
    );
    assert.equal(branch, "mcode/fix-toast-copy-3");
  });

  it("falls back to mcode/update when no preferred name is provided", () => {
    const branch = resolveDefaultCreateBranchName(["main"]);
    assert.equal(branch, "mcode/update");
  });
});
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "mcode/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "mcode/deadbeef";
