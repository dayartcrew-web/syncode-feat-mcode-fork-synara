...
import { buildMCodeBranchName } from "@t3tools/shared/git";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for MCode, then try again.";
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildMCodeBranchName(input.associatedWorktreeBranch ?? input.title);
}
