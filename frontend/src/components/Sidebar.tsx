...
type DebugFeatureFlagsWindow = Window & {
  mcodeShowFeatureFlags?: () => void;
  mcodeHideFeatureFlags?: () => void;
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    debugWindow.mcodeShowFeatureFlags = showFeatureFlags;
    debugWindow.mcodeHideFeatureFlags = hideFeatureFlags;
    ...
    return () => {
      ...
      if (debugWindow.mcodeShowFeatureFlags === showFeatureFlags) {
        delete debugWindow.mcodeShowFeatureFlags;
      }
      if (debugWindow.mcodeHideFeatureFlags === hideFeatureFlags) {
        delete debugWindow.mcodeHideFeatureFlags;
      }
  ...
  // when no MCode-owned run needs near-real-time reconciliation.
  ...
  const projectRunServerByProjectId = useMemo(() => {
    ...
    // 1. Authoritative: MCode-tracked runs matched by pid/ppid.
    ...
    //    servers started outside MCode still light up the running indicator.
  ...
  function renderProjectItem(
    ...
    // A project reads as "running" when MCode tracks a run for it or when a
    // local server (possibly started outside MCode) is attributed by cwd.
        ...
        .then((nextState) => {
          ...
          if (nextState.status === "available") {
            toastManager.add({
              ...
              description: `MCode is preparing version ${nextState.availableVersion ?? "available"} in the background.`,
            });
            return;
          }
          ...
          if (nextState.status === "downloading") {
            toastManager.add({
              type: "info",
              title: "Preparing update",
              description: "MCode is downloading the update in the background.",
            });
            return;
          }
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              ...
              description: `MCode ${nextState.currentVersion} is already the newest version.`,
            });
            return;
          }
...
function SidebarSearchPaletteController(props: {
  ...
  ).filter((_provider, index) => supportsThreadImport(importProviderCapabilityQueries[index]?.data));
