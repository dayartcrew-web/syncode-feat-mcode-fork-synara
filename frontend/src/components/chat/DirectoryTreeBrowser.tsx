...
export const DirectoryTreeBrowser = memo(function DirectoryTreeBrowser({
  ...
  // Track paths that failed to load so we don't retry them on every render
  // (which would cause an infinite render loop when the WS RPC keeps failing).
  const failedPathsRef = useRef<ReadonlySet<string>>(new Set());
  // Track in-flight requests to avoid duplicate concurrent loads of the same path.
  ...
  const loadDirectory = useCallback(
    async (relativePath = "") => {
      ...
      if (entriesByParent[relativePath] || failedPathsRef.current.has(relativePath) || inFlightRef.current.has(relativePath)) {
        return;
      }

      inFlightRef.current = new Set(inFlightRef.current).add(relativePath);
      ...
      try {
        ...
        failedPathsRef.current = new Set(failedPathsRef.current).add(relativePath);
  ...
  const handleEnsureRootLoaded = useCallback(() => {
    if (rootEntries.length === 0 && !loadingPaths.has("") && !failedPathsRef.current.has("")) {
      void loadDirectory();
    }
