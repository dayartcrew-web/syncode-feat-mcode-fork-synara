...
fn init_repo() -> (TempDir, Box<dyn GitService>) {
    ...
    Command::new("git")
        ...
        .expect("git init");
    Command::new("git")
        .args(["config", "user.email", "e2e@syncode.test"])
        .current_dir(path)
        .status()
        .expect("git config email");
    Command::new("git")
        .args(["config", "user.name", "Syncode E2E"])
        .current_dir(path)
        .status()
    ...
    let svc: Box<dyn GitService> =
        Box::new(syncode_git::service::Git2Service::open(path).expect("open repo"));
    (dir, svc)
}
...
fn git_real_repo_branch_create_checkout_delete() {
    ...
    svc.create_branch("feature/test-e2e", false)
    ...
    svc.delete_branch("feature/test-e2e")
